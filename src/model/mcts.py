...
class Node:
    ...
    def select(self, c_puct):
        ...
        best_action = None  # Initialize as None instead of -1
        best_child = None
        ...
        if best_action is None:
            raise ValueError("No valid action found during selection phase")
...
class MCTS:
    ...
    def run(self, root_state):
        ...
        for _ in range(self.n_simulations):
            ...
            while node.expanded and len(node.children) > 0:  # Ensure node has children
            ...
            if current_state.is_terminal():
                ...
                if policy_sum > 0:
                    ...
                    policy_normalized = [(a, 1.0 / len(policy_legal)) for a, _ in policy_legal]
        ...
        # Make sure we have valid visit counts
        if not visit_counts or sum_visits == 0:
            # Fallback to uniform policy over legal actions
            legal_actions = root_state.legal_actions()
            if legal_actions:
